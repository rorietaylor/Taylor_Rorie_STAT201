...
print(paste0("The largest win spread is ", win_spread, ", when the Bears scored ", bear_score, " points and the Opponent scored ",opponent_score, " points."))
```
...
new_bears_scores <- c(37, 9)
new_opponent_scores <- c(17, 17)
new_scores <- cbind(new_bears_scores, new_opponent_scores)
...
for(i in 1:nrow(game_score)){
  if(game_score[i, 1] > game_score[i, 2]){
    print("Win")
  } else if(game_score[i, 2] > game_score[i, 1]){
    print("Loss")
  }
}
```
...
for(i in 1:nrow(game_score)){
  bears_win_check <- as.logical(game_score[i, 1] > game_score[i, 2])
  win <- c(win, bears_win_check)
}
...
for(i in 1:nrow(game_score)){
  if(game_score[i, 1] < game_score[i, 2]){
    num_games_lost <- num_games_lost + 1
  } else if(game_score[i, 1] == game_score[i, 2]){
...
```{r}
number_of_factors <- function(input_integer) {
  num_factors <- 0
...
```{r}
area_of_rectangle <- function(length = 48, width = 48){
  area_sqin <- length * width
  area_sqft <- area_sqin / 144
  
  area_sqin_sqft <- c(area_sqin, area_sqft)
  return(area_sqin_sqft)
}

area_of_rectangle_result <- area_of_rectangle(102, 60)
print(paste0("The area in square inches is ", area_of_rectangle_result[1], "."))
print(paste0("The area in square feet is ", area_of_rectangle_result[2], "."))
...
```{r}
most_letters <- function(input_vector){
  max <- max(nchar(input_vector))
  most_letters_vector <- c()
  
  for(i in input_vector){
    if(nchar(i) == max){
      most_letters_vector <- c(most_letters_vector, i)
    }
  }
  return(most_letters_vector)
}
...
```{r}
value_in_vector <- function(input_value, input_vector){
  in_vector <- FALSE
...
value_in_vector_result <- value_in_vector(value, check)

if(value_in_vector_result == TRUE){
  print(paste(value, "is in the vector."))
} else if(value_in_vector_result == FALSE){
...
```{r}
matrix_na <- function(input_matrix){
...
```{r}
matrix_even_one <- function(input_matrix){
  
  for (i in 1: nrow(input_matrix)){
    for (j in 1: ncol(input_matrix)) {
      if(input_matrix[i, j]%%2 == 0){
        input_matrix[i, j] <- 1
      } else if(input_matrix[i, j]%%2 != 0){
          input_matrix[i, j] <- 0
        }
      }
    }
